...
class Runner:
    ...
    def run_epoch(self, tracker: Optional[ExperimentTracker] = None, epoch_id = None) -> None:
        ...
        for step_i, local_batch in enumerate(tqdm(self.data_loader)):
            ...
            target_tokens = outputs["target_tokens_croped"]
            out_tokens = outputs["out_tokens_croped"]

            target_texts = model.tokenizer.decode(
                target_tokens, skip_special_tokens=True)
            predictions_texts = model.tokenizer.decode(
                out_tokens, skip_special_tokens=True)

            # print("Target:", target_texts)
            # print("Prediction:", predictions_texts)
            for metric in self.metrics:
                if metric.inpyt_type == "str":
                    val = metric.calculate_and_update(
                        ["".join(target_texts)], ["".join(predictions_texts)])
                else:
