...
"""
Dataset class thought for dialogue generation task.

As input text only the context text is used, no answers are 
provided. The dataset outputs the target dialogue, as well as
the list of posible answers to be able to compare the generated
text with the possible answers, and select the closest, for 
this reason is meant to be used for evaluation..
"""
