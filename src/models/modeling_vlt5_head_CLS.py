...
In this code, we modify the original adding a CLS token in the transformer encoder input and
change the transformer decoder by a classification head with LeakyReLU activation function.
"""
...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        # Add a [CLS] to the input ids
        # print('CLS token size', self.CLS_token.size())
        # print('inputs_embeds size', inputs_embeds.size())
        inputs_embeds = torch.cat([self.CLS_token.repeat(B, 1, 1), inputs_embeds], dim=1)
        # print('inputs_embeds size after CLS', inputs_embeds.size())
        B, L = inputs_embeds.size()[:-1] # Update the length of the input sequence with the CLS token
        ...
        if attention_mask is None:
            ...
            mask_CLS = torch.ones((B, 1), dtype=inputs_embeds.dtype, device=inputs_embeds.device)
            attention_mask = torch.cat([mask_CLS, attention_mask], dim=1)
        ...
        if self.config.num_layers > 0:
            ...
            # position_bias[:, :, :, L:] = 0
            # print('position_bias size', position_bias.size())
            # print('extended attention_mask size', extended_attention_mask.size())
...
class VLT5(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        #------------------#
        self.drop = nn.Dropout(config.dropout_input_head)

        self.classification_head = nn.Sequential(
            nn.Linear(config.d_model, 600),
            nn.BatchNorm1d(600),
            nn.LeakyReLU(),
            #nn.Dropout(config.dropout_in_head),
            nn.Linear(600, 256),
            nn.BatchNorm1d(256),
            nn.LeakyReLU(),
            #nn.Dropout(config.dropout_in_head),
        ...
        self.init_weights()
        # Initialize classification head with xavier distribution
    ...
    def create_model_config(cls, args: Any) -> T5Config:
        ...
        config.dropout_input_head = args.dropout_input_head
        config.dropout_in_head = args.dropout_in_head
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states_pooled = self.drop(hidden_states_pooled)
