...
if __name__ == "__main__":
    ...
    parser.add_argument('--batchsize', default=75, type=int, help='batch_size')
    ...
    # Load the pretarined VIT b 16 model
    weights = ViT_B_16_Weights.IMAGENET1K_V1
    transforms_VIT = weights.transforms()
    model = vit_b_16(weights=weights)

    # Define the feature extractor, returns the features from the last layer before the classification head
    from torchvision.models.feature_extraction import create_feature_extractor
    feature_extractor = create_feature_extractor(model, return_nodes=['getitem_5']).to("cuda")
    print(feature_extractor)

    feature_extractor.eval()


    dataset = ComicsRawImagesDataset(image_paths, ocr_file, transforms_VIT)
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            features = feature_extractor(imgs)['getitem_5']
    ...
    import pickle
    with open(os.path.join(out_dir, 'vit_b_16_panels_features.pkl'), 'wb') as f:
